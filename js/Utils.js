...
method:
	...
	expand(box): this; //扩容; 把box合并到this
	...
	computeOverflow(b: Box, r: Box): r; //this相对b超出的部分赋值到r; r的size小于或等于0的话说明完全超出
...
class Box{
	...
	copy(box){
		...
		this.w = box.w;
		this.h = box.h;
		return this; //this.set(box.x, box.y, box.w, box.h);
	}
	...
	randomLoca(box){ //位置在box范围内设置随机数; box 默认this
		box = box || this;
		this.x = Math.random() * (box.x + box.w - box.x) + box.x;
		this.y = Math.random() * (box.y + box.h - box.y) + box.y;
		return this;
	}
	...
	floatSize(n){ //宽高保留n位浮点数
		n = Math.pow(10, n);
		this.w = Math.floor(this.w * n) / n;
		this.h = Math.floor(this.h * n) / n;
		return this;
	}
...
class CanvasAnimateCustom extends CanvasAnimate{
	...
	drawImage(img, x, y, w, h, x1, y1, w1, h1){
		...
		this.context.drawImage(img, x || 0, y || 0, w || img.width, h || img.height, x1, y1, w1, h1);
